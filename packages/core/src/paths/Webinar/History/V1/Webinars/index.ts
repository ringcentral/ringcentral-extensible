...
class Index {
   ...
   */
  public async get(restRequestConfig?: RestRequestConfig): Promise<HistoryWebinarResource> {
    if (this.webinarId === null)
    {
        throw new Error('webinarId must be specified.');
    }
    const r = await this.rc.get<HistoryWebinarResource>(this.path(), undefined, restRequestConfig);
    return r.data;
  }
