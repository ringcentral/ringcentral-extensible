...
class Index {
   ...
   */
  public async get(restRequestConfig?: RestRequestConfig): Promise<HistorySessionResource> {
    if (this.sessionId === null)
    {
        throw new Error('sessionId must be specified.');
    }
    const r = await this.rc.get<HistorySessionResource>(this.path(), undefined, restRequestConfig);
    return r.data;
  }
