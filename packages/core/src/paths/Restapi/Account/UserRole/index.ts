...
class Index {
   ...
   */
  public async post(roleResource: RoleResource, queryParams?: CreateCustomRoleParameters, restRequestConfig?: RestRequestConfig): Promise<string> {
    const r = await this.rc.post<string>(this.path(false), roleResource, queryParams, restRequestConfig);
    return r.data;
  }
   ...
   * User Permission: EditUserRoles
   */
  public async put(roleResource: RoleResource, queryParams?: UpdateUserRoleParameters, restRequestConfig?: RestRequestConfig): Promise<RoleResource> {
    if (this.roleId === null)
    {
        throw new Error('roleId must be specified.');
    }
    const r = await this.rc.put<RoleResource>(this.path(), roleResource, queryParams, restRequestConfig);
    return r.data;
  }
