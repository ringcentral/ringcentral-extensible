 ...
 * 
   ...
   * User Permission: ConfigureEmergencyMaps
   */
  public async post(switchInfo: SwitchInfo, restRequestConfig?: RestRequestConfig): Promise<SwitchInfo> {
    const r = await this.rc.post<SwitchInfo>(this.path(false), switchInfo, undefined, restRequestConfig);
    return r.data;
  }
 ...
 * 
   ...
   * User Permission: ConfigureEmergencyMaps
   */
  public async put(switchInfo: SwitchInfo, restRequestConfig?: RestRequestConfig): Promise<SwitchInfo> {
    if (this.switchId === null)
    {
        throw new Error('switchId must be specified.');
    }
    const r = await this.rc.put<SwitchInfo>(this.path(), switchInfo, undefined, restRequestConfig);
    return r.data;
  }
