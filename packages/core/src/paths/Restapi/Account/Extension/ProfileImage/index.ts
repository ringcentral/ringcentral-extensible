 ...
 * 
   ...
   */
  public async list(restRequestConfig?: RestRequestConfig): Promise<Uint8Array> {
    const r = await this.rc.get<Uint8Array>(this.path(false), undefined, {...restRequestConfig, responseType: 'arraybuffer'});
    return r.data;
  }
 ...
 * 
   ...
   * App Permission: ReadAccounts
   */
  public async get(queryParams?: ReadScaledProfileImageParameters, restRequestConfig?: RestRequestConfig): Promise<Uint8Array> {
    if (this.scaleSize === null)
    {
        throw new Error('scaleSize must be specified.');
    }
    const r = await this.rc.get<Uint8Array>(this.path(), queryParams, {...restRequestConfig, responseType: 'arraybuffer'});
    return r.data;
  }
}
