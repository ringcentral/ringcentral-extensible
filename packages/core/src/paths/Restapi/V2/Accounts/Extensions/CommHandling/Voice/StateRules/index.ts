 ...
 * 
   ...
   */
  public async list(queryParams?: ListVoiceStateBasedRulesParameters, restRequestConfig?: RestRequestConfig): Promise<CommStateBasedRulesResource> {
    const r = await this.rc.get<CommStateBasedRulesResource>(this.path(false), queryParams, restRequestConfig);
    return r.data;
  }
 ...
 * 
   ...
   * App Permission: ReadAccounts
   */
  public async get(restRequestConfig?: RestRequestConfig): Promise<CommStateBasedRuleResource> {
    if (this.stateId === null)
    {
        throw new Error('stateId must be specified.');
    }
    const r = await this.rc.get<CommStateBasedRuleResource>(this.path(), undefined, restRequestConfig);
    return r.data;
  }
 ...
 * 
   ...
   */
  public async patch(commStateBasedRuleUpdateRequest: CommStateBasedRuleUpdateRequest, restRequestConfig?: RestRequestConfig): Promise<CommStateBasedRuleResource> {
    if (this.stateId === null)
    {
        throw new Error('stateId must be specified.');
    }
    const r = await this.rc.patch<CommStateBasedRuleResource>(this.path(), commStateBasedRuleUpdateRequest, undefined, restRequestConfig);
    return r.data;
  }
}
