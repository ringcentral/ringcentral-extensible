 ...
 * 
   ...
   * Rate Limit Group: Light
   */
  public async get(restRequestConfig?: RestRequestConfig): Promise<TimezoneInfoBase> {
    if (this.timezoneId === null)
    {
        throw new Error('timezoneId must be specified.');
    }
    const r = await this.rc.get<TimezoneInfoBase>(this.path(), undefined, restRequestConfig);
    return r.data;
  }
}
